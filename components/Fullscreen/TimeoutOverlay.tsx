...
export const TimeoutOverlay: React.FC<TimeoutOverlayProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center isolate bg-slate-950/90 backdrop-blur-2xl p-safe">
        
        {/* Minimize Button - Explicitly Top-Right Safe Area */}
        <button 
            onClick={onMinimize}
            className="absolute top-6 right-6 p-3 rounded-full bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all z-[110] active:scale-95"
        ...
        <div className="flex flex-col landscape:flex-row items-center justify-center gap-8 landscape:gap-16 w-full max-w-5xl h-full max-h-screen overflow-y-auto landscape:overflow-hidden p-6 relative z-10">
            ...
            <div className="flex flex-col items-center shrink-0">
                ...
                >
                    <div className={`px-4 py-1.5 rounded-full border ${theme.border} ${theme.bg} ${theme.text} font-black text-xs uppercase tracking-[0.2em] shadow-[0_0_20px_-5px_currentColor]`}>
