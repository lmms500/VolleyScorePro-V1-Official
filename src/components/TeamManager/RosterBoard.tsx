...
import { Layers, Search, X, List, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const RosterBoard: React.FC<RosterBoardProps> = ({ courtLimit, benchLimit }) => {
    ...
    const { reorderQueue, disbandTeam } = useActions();
                        ...
                        {queue.length === 0 ? (
                            ...
                            <AnimatePresence initial={false} mode="popLayout">
                                {queue.map((team: Team, idx: number) => {
                                    ...
                                    if (!isVisible) return <div key={team.id} className="snap-center w-80 sm:w-96 flex-shrink-0 h-full" />;
