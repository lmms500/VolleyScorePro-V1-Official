...
class AudioService {
  ...
  private setupUnlockListener() {
    const unlock = async () => {
      ...
      if (this.ctx && (this.ctx.state as any) !== 'running' && (this.ctx.state === 'suspended' || (this.ctx.state as any) === 'interrupted')) {
        try {
          await this.ctx.resume();
          if ((this.ctx.state as any) === 'running') {
            this.preWarm(this.ctx);
          }
        } catch (e) {
