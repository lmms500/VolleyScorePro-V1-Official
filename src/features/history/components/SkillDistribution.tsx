...
export const SkillDistribution: React.FC<SkillDistributionProps> = ({ match, teamStats, distribution }) => {
    ...
    const createPiePath = (stats: TeamStat, startAngle: number = -90): { segments: { skill: string; percent: number; color: string; pathD: string; startAngle: number; endAngle: number }[] } => {
        const total = stats.attack + stats.block + stats.ace + stats.opponent_error;
        if (total === 0) return { segments: [] };

        const segments: { skill: string; percent: number; color: string; pathD: string; startAngle: number; endAngle: number }[] = [];
        const skills = [
        ...
        skills.forEach(skill => {
            ...
            let pathD = '';
            if (angle > 359.9) {
                pathD = `M ${cx} ${cy - radius} A ${radius} ${radius} 0 1 1 ${cx} ${cy + radius} A ${radius} ${radius} 0 1 1 ${cx} ${cy - radius} Z`;
            } else {
            ...
            <div className="flex items-center justify-around gap-4">
                <div className="flex flex-col items-center gap-2">
                    <svg viewBox="0 0 100 100" className="w-24 h-24">
                        ...
                        {pieA.segments.map((seg, i) => (
                            <path key={i} d={seg.pathD} fill={seg.color} fillOpacity="0.8" stroke="transparent" strokeWidth="0" />
                        ))}
