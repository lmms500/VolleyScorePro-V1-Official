...
const RotationControls = memo(({ 
    ...
    <div className={`flex flex-col gap-1 pointer-events-auto ${align === 'left' ? 'items-start' : 'items-end'}`}>
        <span className="text-[8px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-300 bg-white/40 dark:bg-white/10 border border-white/30 dark:border-white/10 px-2.5 py-0.5 rounded-full backdrop-blur-md mb-1 shadow-sm">
            {teamName}
        </span>
        <div className="flex gap-1.5">
            ...
            <button onClick={onRotateClockwise} className={`w-10 h-10 rounded-xl ${theme.bg} hover:${theme.bg.replace('/20', '/35')} border ${theme.border} flex items-center justify-center backdrop-blur-md active:scale-95 transition-all ${theme.text} dark:${theme.textDark} shadow-[0_4px_12px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.4)] hover:shadow-lg border-white/20`}>
...
export const CourtFooter: React.FC<CourtFooterProps> = memo(({
    ...
    return (
        <div className="w-full px-4 pb-safe-bottom pt-1 mb-2 shrink-0 flex justify-between items-end relative z-40 bg-transparent pointer-events-auto">
            <RotationControls 
                teamName={teamA.name} 
                theme={themeA} 
                align="left" 
