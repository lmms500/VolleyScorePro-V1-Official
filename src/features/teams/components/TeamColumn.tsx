        ...
        <div ref={droppableRef} className={containerClass}>
                ...
                <div className="flex items-center justify-between border-t border-slate-200 dark:border-white/5 pt-2 mt-1 mb-2 w-full">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <div className="relative">
                            ...
                            <AnimatePresence>
                                {showSortMenu && (
                                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} className="absolute right-0 top-full mt-2 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.2),inset_0_1px_0_0_rgba(255,255,255,0.15)] border border-black/5 dark:border-white/10 ring-1 ring-inset ring-white/10 p-1 flex flex-col min-w-[140px]">
            ...
            {team.hasActiveBench && (
                <div className="flex justify-end mb-2 gap-2 px-1">
                    <button onClick={() => setIsSubModalOpen(true)} className={`flex items-center justify-center p-2 rounded-xl text-[9px] font-bold uppercase tracking-wider transition-all bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/5 ring-1 ring-inset ring-white/10 text-slate-500 hover:text-indigo-500 shadow-sm active:scale-95`} title="Substitute Player"><ArrowRightLeft size={16} /></button>
                    <button onClick={toggleView} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[9px] font-bold uppercase tracking-wider transition-all active:scale-95 ${viewMode === 'reserves' ? 'bg-gradient-to-br from-amber-500 to-amber-600 text-white shadow-lg shadow-amber-500/20 ring-1 ring-inset ring-white/10' : 'bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>
